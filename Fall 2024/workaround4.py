...
# Load sequence for larger N
N = 4  # Adjust this as needed to fit within QPU time limits
...
# Quantum Annealing with Smaller Rounds
max_num_reads = 1000  # Reduce num_reads to stay within QPU limits
num_rounds = math.ceil(19**N / max_num_reads)
samples_per_round = min(max_num_reads, 19**N // num_rounds)
...
for _ in range(num_rounds):
    ...
    response = sampler.sample_qubo(Q, num_reads=samples_per_round)
    ...
    if len(explored_variants_quantum) >= 19**N:
        break
...
for variant in product(aa_list, repeat=N):
...
output_filename = f"mutation_comparison_output_N{N}.txt"
